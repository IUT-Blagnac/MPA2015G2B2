...
casper.thenClick('#weeknavbarA .weekprevious',function(){
    this.test.comment('Click <');
    if ( CAPTURE ) { this.capture("scheduleview_accueil_previous.png"); }
    if ( CAPTURE ) { this.captureSelector("scheduleview_weeknavbarA_previous.png", '#weeknavbarA'); }
    if ( DUMP ) { utils.dump(this.getElementInfo('#weeknavbarA .weekprevious')); }
...
casper.then(function() {
 ...
 casper.each(el_w_m, function(self, sel) {
  casper.thenClick('#weeknavbarA '+sel[0],function(){
